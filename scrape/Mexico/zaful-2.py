...
dfLinks = return_chunk(dfLinks,2)
...
for index,row in dfLinks.iterrows():
    ...
    try:
        ...
        df_save.to_excel('zaful2.xlsx')
        
        file = open("zaful_chunk2.txt", "w")
...
file = open("/home/aa/cloudJ/JB/PRD/scrapper/Scripts/Mexico/zaful-2.txt", "w")
file.write(  os.linesep +f'ZAFUL_2 - {len(df)}' ) 
file.write(  os.linesep +f'CANTIDAD DE ITEMS - {len(df)}' ) 
file.write(  os.linesep +f'DURACION - {format(end_ - start_)[:-4]}' ) 
file.close()


df.to_excel(f'/home/aa/cloudJ/JB/PRD/scrapper/Scripts/Mexico/Salida/zaful-2{fecha}.xlsx')

print('TERMINO ZAFUL-2.py')